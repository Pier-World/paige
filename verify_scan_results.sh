...
echo "Fetching most recent detected trips..."
echo ""

curl -k -X GET \
  "$PROJECT_URL/rest/v1/potential_trips?user_id=eq.$USER_ID&detection_source=eq.calendar&order=created_at.desc&limit=5&select=id,destination,destination_city,start_date,end_date,confidence_score,trip_type,status,metadata" \
  -H "Authorization: Bearer $ANON_KEY" \
  -H "apikey: $ANON_KEY" \
  -H "Content-Type: application/json" \
  -H "Prefer: return=representation" 2>&1 | head -50
